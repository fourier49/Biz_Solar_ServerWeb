...
<div id="title">Junction BOX</div>

<table id="subTitle"><tr>
<th>
<a href="<?=site_url("admin/jb/dis");?>">Disable</a>
</th>
<td id="jb_select">
</td>
<td>
<form method="post" action="<?=site_url("admin/jb/s");?>">
Mac : DC<input type="text" name="jb_mac" value="<?if(!empty($jb_mac)) echo $jb_mac;?>" size=10>
...
<table id="list">
	...
	<tr>
		...
		<td><?if(!empty($row['client_id']) && $row['client_id'] > 10)?>
		<a href="<?=site_url("admin/area_block/0/".$row['am_id'])?>">A8<?=$row['am_mac']?></a>
		<??></td>
	</tr>
	<?	$i++;} ?>
</table>
